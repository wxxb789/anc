...
import { REDIRECT_LIMIT } from '../src/lib/routes.ts';
...
test('a corpus larger than the redirect ceiling fails before the build runs', () => {
  // `renderRedirects` already throws past 2,000 rules — but only after
  // `astro build` has written a whole `dist/`. This is the same ceiling checked
  // at validation time, which is the first thing the build chain runs.
  ...
  // The ceiling must actually keep the redirect map inside the host's limit,
  // which is the reason it exists: two rules per entry.
  assert.ok(
    MAX_ENTRIES * 2 <= REDIRECT_LIMIT,
    `${MAX_ENTRIES} entries emit ${MAX_ENTRIES * 2} rules, over the ${REDIRECT_LIMIT} rule ceiling`,
  );
