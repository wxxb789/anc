...
| The GitHub Action's dependency install | `npm install` in the action path resolves the caret ranges fresh, so a dependency minor can reach a user's build with no diff in their repository. The *generator* is pinned exactly by the ref in their `uses:`; its tree is not | unassigned |
