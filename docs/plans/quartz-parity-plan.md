...
| C5 | **Deny-by-default assets, assertable as a test** | There is no `**` copy step anywhere. Anything in `dist/` traces to the manifest or to `public/`, so "every published byte has a manifest entry" is a gate TK-09 can write | Quartz's own docs state the opposite behaviour as fact, and `plugins/emitters/assets.ts` implements it. Their privacy failure was never the Markdown — it was the images. Fixing it means deleting the asset emitter |
