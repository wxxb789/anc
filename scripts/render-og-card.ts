...
/**
 * The card is one image shared by every route, including the Chinese ones, so
 * its subtitle is the navigation language — the same rule every non-document
 * surface follows. A per-language card would be a second image and a second
 * `og:image` to choose between, which is a design decision this ticket has no
 * reason to make.
 */
const CARD_LANGUAGE = NAV_LANGUAGE;
const SUBTITLE = translate(CARD_LANGUAGE).siteSubtitle;
